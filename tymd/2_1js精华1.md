...
// 创建时候添加
var arr = [1，2，3];
// 数据中的元素可以是任意类型字符串 数字 bool unll undefined 对象
arr = ["hello",1,true ,null ,undefined,{name :"孙悟空"}];
//函数
...
foreach  需要一个函数作为参数
//我们创建，但不由我们调取的称为回调函数
// 有几个元素就执行几次，每次执行，会将遍历的
// 元素以实参形式 传递进来。我们可以定义形参，来读取
// 传递3个参数 
	//1.当前遍历数组元素 value
	//2.当前遍历数组索引 index
	//3.当前遍历数组	   obj
...
```javascript
var num = new Number(3);// 将数字转object
var str = new String("123");//字符串转object
var bool = new Boolean(true);//bool转object
num.hello = "123";
console.log( typof bool);
```
